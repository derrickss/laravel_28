...
<div class="gome_container">
	...
	<div class="prd_firstscreen_left">
		...
		<div id="magnifier">
		    <div class="small-box">
		      	<img src="https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1539534244319&di=e5ceece5581ed34b233c4981b5e7a5d6&imgtype=0&src=http%3A%2F%2Fimg.zcool.cn%2Fcommunity%2F016f7a556822070000012b207f75b7.jpg%40900w_1l_2o_100sh.jpg" width="100%" alt="#">
		    ...
		    <div class="thumbnail-box">
		      	...
		      	<div class="list">
		        	<ul class="wrapper">
		          		<li class="item item-cur" data-src="{{$data[0]->g_pic}}"><img src="{{$data[0]->g_pic}}" alt="#"></li>
			          	<li class="item" data-src="{{$data[0]->g_pic}}"><img src="{{$data[0]->g_pic}}" alt="#"></li>
			          	<li class="item" data-src="{{$data[0]->g_pic}}"><img src="{{$data[0]->g_pic}}" alt="#"></li>
			          	<li class="item item-cur" data-src="{{$data[0]->g_pic}}"><img src="{{$data[0]->g_pic}}" alt="#"></li>
			          	<li class="item" data-src="{{$data[0]->g_pic}}"><img src="{{$data[0]->g_pic}}" alt="#"></li>
		        	</ul>
		    ...
		    <div class="big-box">
		      	<img src="https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1539534244319&di=e5ceece5581ed34b233c4981b5e7a5d6&imgtype=0&src=http%3A%2F%2Fimg.zcool.cn%2Fcommunity%2F016f7a556822070000012b207f75b7.jpg%40900w_1l_2o_100sh.jpg" alt="#">
		    </div>
		</div>
	...
	<div class="prd_firstscreen_center">
    	...
    	<div class="prd_price_1">
            <div class="unitprice">
                ...
                <p>
            		...
            		<span class="price"><em>¥</em>{{$data[0]->price}}</span>
            	</p>
            </div>
        ...
        <div class="prd_properties">
	        ...
	        <div class="prd-properties-2">
	        	...
	        	<!-- 把商品id传过去 -->
	        	<input type="hidden" value="{{$data[0]->id}}" name="gid">
                <div class="prd_properties_other">
                    <label class="prdLeft">{{$value->name}}:</label>
                    <div class="prdRight">
                    	@foreach($value->dev as $val)
                        <div class="prdcol">
                            <a href="javascript:void(0);" class="danxuan{{$value->id}}" class="" title="{{$val->text}}" data-alt="{{$val->text}}" value="1" id="">
                            		<input id="input" type='hidden' value="{{$val->text}}" name="style[{{$value->id}}]">
                                <span>{{$val->text}}</span>
                            </a>


              <script>
	        		$(".danxuan{{$value->id}}").click(function(){
	        			$(".danxuan{{$value->id}}").attr("class","danxuan{{$value->id}}");
	        			$(".danxuan{{$value->id}}").attr("id","");
	        			$("#select{{$value->id}}").attr("class","danxuan{{$value->id}}");
	        			$(this).attr("class","select");
	        			$(".select").children('span').append("<i></i>");
	        			$(".select").children('#input').removeAttr("disabled");
	        			$(".danxuan{{$value->id}}").children('#input').attr('disabled','disabled');
	        			$(this).attr("id","select{{$value->id}}");
	    ...
	    <div class="prd_buttons">
		    <div class="count-wrapper" id="btnCount">
		    		<input type="number" class="quantity" value="1" name="num" min="1">
		    </div>
		   <button type="submit" name="mai" value="1" style="border:0;" ><a class="btn-product" style="width:79.5%;margin-right:30px;background: #ffeded;color: #3c4042;">立即购买</a></button>
		    <button type="submit" name="mai" value="2" style="border:0;" ><a class="btn-product" style="width:79.5%;">加入购物车</a></button>
	    </div>
	    {{csrf_field()}}
	  </form>
	  <!-- 判断选择参数是否齐全 -->
...
<p>
		...
		<a rel="example_group" href="/images/example/12_b.jpg" title=""><img class="last" alt="" src="/images/example/12_s.jpg" /></a>
										...
										<br/>
										@if($val->reply)
										<div style="border: 1px solid #f40;width:250px;padding:15px; ">商家回复:{{$val->reply}}</div>
										@endif
										<div id="content">
									...
									<div class="reply-right">
										...
										<span class="reply_avatar_userName">{{$val->user_name}}</span>
										</div>
