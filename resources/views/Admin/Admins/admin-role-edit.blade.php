	...
	<form action="/saveauth" method="post" class="form form-horizontal" id="form-admin-role-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$role->name}}" placeholder="" id="roleName" name="roleName" disabled="disabled">
			</div>
		</div>
					...
					<dd>
						<dl class="cl permission-list2">
							...
							<dd>
								<label class="">
								@foreach($auth as $k)
									<input type="checkbox" value="{{$k->id}}" @if(in_array($k->id,$nids)) checked @endif name="nids[]" id="user-Character-0-0-0">
									{{$k->name}}</label>
								@endforeach
