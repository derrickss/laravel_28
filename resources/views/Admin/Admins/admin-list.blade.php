...
<nav class="breadcrumb">
  ...
  <div class="page-container">
   ...
   <div class="cl pd-5 bg-1 bk-gray mt-20">
    <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont"></i> 批量删除</a> <a href="javascript:;" onclick="admin_add('添加管理员','admin-list/create','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont"></i> 添加管理员</a></span>
   ...
   <table class="table table-border table-bordered table-bg">
    ...
    @foreach($admin as $k)
     ...
     <!--  <td>13000000000</td>
      ...
      <td>{{$k->level}}</td>
      <td>{{$k->add_time}}</td>
      <td class="td-status"><span class="label label-success radius">{{$k->status}}</span></td>
      <td class="td-manage">
		<!-- 判断状态呀 -->
		@if($k->status == '已启用')
	...
	<a onClick="admin_start(this,'{{$k->id}}',1)" href="javascript:;" title="启用" style="text-decoration:none"><i class="Hui-iconfont">&#xe615;</i></a>
		...
		@endif
		<!-- 判断状态结束 -->
      <a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','admin-list/{{$k->id}}/edit','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont"></i></a>

      <a style="text-decoration:none" class="ml-5" onclick="change_password('修改密码','admin-listchange/{{$k->id}}/edit','{{$k->id}}','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont"></i></a>
      <a title="删除" href="javascript:;" onclick=""  class="ml-5 del" style="text-decoration:none"><i class="Hui-iconfont"></i></a></td>
     </tr>
    @endforeach
    </tbody>
   </table>
...
function admin_stop(obj,id,sta){
	layer.confirm('确认要停用吗？',function(index){
		// alert(id);
		$.get('admin-liststatus/',{id:id,sta:sta},function(data) {
...
<script type="text/javascript">
    ...
    if(ss){
    //         // alert(id);
        $.get("/admin-listdel",{id:id},function(data){
          // alert(data);
          if(data==1){
            alert("删除成功");
            //删除数据所在的tr
